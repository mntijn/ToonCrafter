...
def batch_ddim_sampling(model, cond, noise_shape, n_samples=1, ddim_steps=50, ddim_eta=1.0,
    ...
    for _ in range(n_samples):
        ...
        batch_images = model.decode_first_stage(
            samples, **additional_decode_kwargs)
        ...
        # reconstruct from latent to pixel space
        batch_images_middle = model.decode_first_stage(
            samples, **additional_decode_kwargs)
        batch_images[:, :, batch_images.shape[2]//2-1:batch_images.shape[2]//2 +
                     1] = batch_images_middle[:, :, batch_images.shape[2]//2-2:batch_images.shape[2]//2]
            ...
            try:
                ...
                for k in list(new_pl_sd.keys()):
                    if "framestride_embed" in k:
                        new_key = k.replace(
                            "framestride_embed", "fps_embedding")
...
def load_video_batch(filepath_list, frame_stride, video_size=(256, 256), video_frames=16):
    ...
    for filepath in filepath_list:
        padding_num = 0
        vidreader = VideoReader(filepath, ctx=cpu(
            0), width=video_size[1], height=video_size[0])
        ...
        frame_tensor = torch.tensor(
            frames.asnumpy()).permute(3, 0, 1, 2).float()
...
def load_image_batch(filepath_list, image_size=(256, 256)):
    ...
    for filepath in filepath_list:
        ...
        if ext == '.mp4':
            vidreader = VideoReader(filepath, ctx=cpu(
                0), width=image_size[1], height=image_size[0])
            frame = vidreader.get_batch([0])
            img_tensor = torch.tensor(frame.asnumpy()).squeeze(
                0).permute(2, 0, 1).float()
...
def save_videos(batch_tensors, savedir, filenames, fps=10):
    ...
    for idx, vid_tensor in enumerate(batch_tensors):
        ...
        frame_grids = [torchvision.utils.make_grid(framesheet, nrow=int(
            n_samples)) for framesheet in video]  # [3, 1*h, n*w]
